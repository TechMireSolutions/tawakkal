...
import { HiOutlineVideoCamera, HiXMark, HiArrowPath } from 'react-icons/hi2';
...
export default function VideoUploader({
  ...
  const handleFile = (file) => {
    ...
    if (accept && !accept.split(',').map((item) => item.trim()).some((type) => file.type === type || type === '*/*' || type === `${file.type.split('/')[0]}/*`)) {
      setError('Invalid file type. Please upload a supported video format.');
      return;
    }
      ...
      >
        ...
        {preview ? (
          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ position: 'relative', maxWidth: '100%', maxHeight: '180px', display: 'flex', justifyContent: 'center' }}>
              <video
                src={preview}
                controls
                style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: 'var(--admin-text-muted)' }}>
            <div style={{
              ...
              <HiOutlineVideoCamera size={24} color="var(--admin-text-muted)" />
            </div>
            <p style={{ margin: '0 0 4px 0', fontSize: '14px', fontWeight: 500, color: 'var(--admin-text)' }}>No video uploaded</p>
            <p style={{ margin: 0, fontSize: '12px' }}>Drag & drop a video here, or click to browse</p>
          </div>
        )}
      </div>
    </div>
